...
class FakeFundMessageEvent(Event):
    
    def __init__(self, message: str, user_id: str = "test_user", group_id: str = "test_group"):
        super().__init__()
    ...
    def get_session_id(self) -> str:
        return f"group_{self._group_id}_{self._user_id}"
    ...
    def get_event_description(self) -> str:
        return "fake fund message event"
...
async def fund_plugin(app: App):
    from nonebot import get_plugin, load_plugin
    ...
    if plugin is None:
        try:
            ...
            pytest.skip(f"无法加载 fund 插件: {e}")
    
    if plugin is None:
        pytest.skip("fund 插件加载失败")
    
    return plugin


@pytest.mark.asyncio
async def test_fund_query_002170_output_consistency(app: App, fund_plugin):
    """测试代码 002170 的输出一致性"""
    ...
    assert code_type == CodeType.OFF_MARKET_FUND, f"002170 应该被识别为场外基金，实际识别为: {code_type}"
    ...
    for code, expected_type in zip(test_codes, expected_types):
        ...
        assert actual_type == expected_type, f"代码 {code} 应该被识别为 {expected_type.value}，实际识别为: {actual_type.value}"


@pytest.mark.asyncio
async def test_fund_query_002170_with_mocked_data(app: App):
...
近1年: 15.30%"""
    ...
    assert formatted_output == expected_baseline_output, f"输出与基准不一致:\n实际输出:\n{formatted_output}\n期望基准:\n{expected_baseline_output}"
...
async def test_fund_code_identification_consistency():
    ...
    for code, expected_type in test_cases:
        ...
        assert code_type == expected_type, f"代码 {code} 应该被识别为 {expected_type.value}，实际识别为: {code_type.value}"
