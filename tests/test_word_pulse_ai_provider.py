"""词频通过真实 SDK 验证共享路由、三协议 JSON mode 与错误绑定。"""
...
def _reply(protocol, content, failure=None):
    ...
    if protocol == "anthropic_messages":
        return {
            ...
            "stop_reason": {"truncated": "max_tokens", "refused": "refusal"}.get(failure, "end_turn"),
        }
    return {
        "status": "incomplete" if failure == "truncated" else "completed",
        "incomplete_details": {"reason": "max_output_tokens"} if failure == "truncated" else None,
        "output": [{
            "type": "message", "id": "msg-1", "role": "assistant", "status": "completed",
            "content": ([{"type": "refusal", "refusal": "拒绝"}] if failure == "refused" else [
...
async def test_word_pulse_operations_use_shared_route_and_json_mode(protocol, operation):
    ...
    payloads = {
        ...
        "summarize": {"ranking": [], "trend": "讨论增加", "examples": [], "unclassified_high_freq": []},
    }
    content = "```json\n" + json.dumps(payloads[operation], ensure_ascii=False) + "\n```"
    with mock_ai_http(lambda request: httpx2.Response(200, json=_reply(protocol, content))) as http:
        if operation == "expand":
            result = await expand_charsets(seeds=["茅台"], theme="炒股")
            assert result == {"茅台": ["茅", "台", "酒", "业", "股"]}
        elif operation == "classify":
            result = await classify_batch(messages=[(1, "茅子涨了")], clusters=[{"name": "茅台"}], theme_name="炒股")
            assert result == [(1, "茅台")]
        else:
...
async def test_word_pulse_rejects_incomplete_or_refused_output(protocol, failure):
    ...
    _install(protocol)
    with (
        mock_ai_http(lambda request: httpx2.Response(200, json=_reply(protocol, '{"results": []}', failure))),
        pytest.raises(WordPulseAIResponseError),
    ):
        await _request_llm(messages=[{"role": "user", "content": "测试"}], temperature=0, timeout_seconds=10)
